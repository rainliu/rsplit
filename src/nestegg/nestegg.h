...
typedef struct {
  ...
  /** User supplied seek callback.
      ...
      @retval -1 Error. */
  int (* seek)(Int64 offset, int whence, void * userdata);

  /** User supplied tell callback.
      @param userdata The #userdata supplied by the user.
      @returns Current position within the stream.
      @retval -1 Error. */
  Int64 (* tell)(void * userdata);
...
/** Query the duration of the media stream in nanoseconds.
    ...
    @retval  0 Success.
    @retval -1 Error. */
int nestegg_duration(nestegg * context, UInt64 * duration);
...
/** Query the tstamp scale of the media stream in nanoseconds.
    ...
    @retval  0 Success.
    @retval -1 Error. */
int nestegg_tstamp_scale(nestegg * context, UInt64 * scale);
...
/** Seek @a track to @a tstamp.  Stream seek will terminate at the earliest
    ...
    @retval  0 Success.
    @retval -1 Error. */
int nestegg_track_seek(nestegg * context, unsigned int track, UInt64 tstamp);
...
/** Query the time stamp in nanoseconds of @a packet.
    ...
    @retval  0 Success.
    @retval -1 Error. */
int nestegg_packet_tstamp(nestegg_packet * packet, UInt64 * tstamp);
