...
#define E_LAST \
  { NULL, 0, TYPE_UNKNOWN, 0, DESC_FLAG_NONE, NULL, 0, 0 }
...
static int
ne_io_seek(nestegg_io * io, Int64 offset, int whence)
{
...
static int
ne_bare_read_vint(nestegg_io * io, UInt64 * value, UInt64 * length, enum vint_mask maskflag)
{
  int r;
  unsigned char b;
  size_t maxlen = 8;
...
static int
ne_read_id(nestegg_io * io, UInt64 * value, UInt64 * length)
{
  return ne_bare_read_vint(io, value, length, MASK_NONE);
}

static int
ne_read_vint(nestegg_io * io, UInt64 * value, UInt64 * length)
{
  return ne_bare_read_vint(io, value, length, MASK_FIRST_BIT);
}

static int
ne_read_svint(nestegg_io * io, Int64 * value, UInt64 * length)
{
  int r;
  UInt64 uvalue;
  UInt64 ulength;
...
static int
ne_read_uint(nestegg_io * io, UInt64 * val, UInt64 length)
{
  unsigned char b;
  int r;
...
static int
ne_read_int(nestegg_io * io, Int64 * val, UInt64 length)
{
  int r;
  UInt64 uval, base;
...
static int
ne_read_float(nestegg_io * io, double * val, UInt64 length)
{
  union {
    UInt64 u;
    float f;
...
static int
ne_read_string(nestegg * ctx, char ** val, UInt64 length)
{
  char * str;
  int r;
  ...
  if (length == 0 || length > LIMIT_STRING)
    return -1;
  str = (char *)ne_pool_alloc((size_t)(length + 1), ctx->alloc_pool);
  r = ne_io_read(ctx->io, (unsigned char *) str, (size_t)length);
  if (r != 1)
    return r;
  str[length] = '\0';
  *val = str;
...
static int
ne_read_binary(nestegg * ctx, struct ebml_binary * val, UInt64 length)
{
  if (length == 0 || length > LIMIT_BINARY)
    return -1;
  val->data = (unsigned char *)ne_pool_alloc((size_t)length, ctx->alloc_pool);
  val->length = (size_t)length;
  return ne_io_read(ctx->io, val->data, (size_t)length);
}

static int
ne_get_uint(struct ebml_type type, UInt64 * value)
{
  if (!type.read)
    return -1;
...
static int
ne_is_ancestor_element(UInt64 id, struct list_node * ancestor)
{
...
ne_find_element(UInt64 id, struct ebml_element_desc * elements)
{
  struct ebml_element_desc * element;
...
static int
ne_peek_element(nestegg * ctx, UInt64 * id, UInt64 * size)
{
  int r;
...
static int
ne_read_element(nestegg * ctx, UInt64 * id, UInt64 * size)
{
  int r;
  ...
  for (;;) {
    ...
    if (element) {
      ...
      if (element->flags & DESC_FLAG_OFFSET) {
        data_offset = (Int64 *) (ctx->ancestor->data + element->data_offset);
...
static int
ne_read_xiph_lace_value(nestegg_io * io, UInt64 * value, size_t * consumed)
{
  int r;
  UInt64 lace;
...
static int
ne_read_xiph_lacing(nestegg_io * io, size_t block, size_t * read, UInt64 n, UInt64 * sizes)
{
  int r;
  size_t i = 0;
  UInt64 sum = 0;
...
static int
ne_read_ebml_lacing(nestegg_io * io, size_t block, size_t * read, UInt64 n, UInt64 * sizes)
{
  int r;
  UInt64 lace, sum, length;
  Int64 slace;
  size_t i = 0;
...
static int
ne_read_block(nestegg * ctx, UInt64 block_id, UInt64 block_size, nestegg_packet ** data)
{
  int r;
  Int64 timecode, abs_timecode;
  nestegg_packet * pkt;
  struct cluster * cluster;
  struct frame * f, * last;
  struct track_entry * entry;
  double track_scale;
  UInt64 track, length, frame_sizes[256], cluster_tc, flags, frames, tc_scale, total;
  ...
  *data = NULL;
  ...
  pkt->track = track - 1;
  pkt->timecode = (UInt64)(abs_timecode * tc_scale * track_scale);
  ...
  for (i = 0; i < frames; ++i) {
    ...
    f = (frame *)ne_alloc(sizeof(*f));
    f->data = (unsigned char *)ne_alloc((size_t)frame_sizes[i]);
    f->length = (size_t)frame_sizes[i];
    r = ne_io_read(ctx->io, f->data, (size_t)frame_sizes[i]);
    if (r != 1) {
      free(f->data);
      free(f);
      nestegg_free_packet(pkt);
...
static struct seek *
ne_find_seek_for_id(struct ebml_list_node * seek_head, UInt64 id)
{
...
ne_find_cue_point_for_tstamp(struct ebml_list_node * cue_point, UInt64 scale, UInt64 tstamp)
{
  UInt64 time;
...
static int
ne_is_suspend_element(UInt64 id)
{
...
int
nestegg_duration(nestegg * ctx, UInt64 * duration)
{
  UInt64 tc_scale;
  ...
  *duration = (UInt64) (unscaled_duration * tc_scale);
  return 0;
}

int
nestegg_tstamp_scale(nestegg * ctx, UInt64 * scale)
{
...
int
nestegg_track_seek(nestegg * ctx, unsigned int track, UInt64 tstamp)
{
  int r;
...
int
nestegg_packet_tstamp(nestegg_packet * pkt, UInt64 * tstamp)
{
  *tstamp = pkt->timecode;
  return 0;
}
...
int nestegg_seek_cb(Int64 offset, int whence, void *userdata) 
{
  switch (whence) {
  ...
  return fseek((FILE *)userdata, (long)offset, whence) ? -1 : 0;
}


Int64 nestegg_tell_cb(void *userdata) 
{
